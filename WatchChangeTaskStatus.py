    ...
    for task_result in db:
        if task_result is not None:
            if db_handler.is_task_status_confirm_from_doing(task_result):
                requests.post(os.getenv("DISCORD_WEBHOOK_TASK_STATUS"), message.confirm_message(task_result))
            elif db_handler.is_task_status_doing_from_confirm(task_result):
                requests.post(os.getenv("DISCORD_WEBHOOK_TASK_STATUS"), message.fb_message(task_result))
